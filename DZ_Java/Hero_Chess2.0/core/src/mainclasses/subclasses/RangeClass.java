    ...
    public void step(ArrayList<BaseClass> enemy, ArrayList<BaseClass> allies) {
        ...
        if (!this.die) {
            if (this.arrows > 0) {
                ...
                Point positionSelf = this.unitpoint;
                Point positionTarget = target.getUnitpoint();
                this.arrows--;
                this.combatLog = this.toString().charAt(0) + " " + this.name + " " + positionSelf + " shot at: " + target.toString().charAt(0) + " " + target.getName() + " " + positionTarget + " damage: " + (hpBefore - hpAfter);
